...
describe("computeWarpGrid", () => {
  ...
  it("fills the entire grid buffer with finite numbers", () => {
    ...
    computeWarpGrid(1.5, PARAMS, fixedState, out);
  ...
  it("is deterministic for identical inputs", () => {
    ...
    computeWarpGrid(2.0, PARAMS, fixedState, a);
    computeWarpGrid(2.0, PARAMS, fixedState, b);
  ...
  it("changes output as animation time advances", () => {
    ...
    computeWarpGrid(0.0, PARAMS, fixedState, t0);
    computeWarpGrid(5.0, PARAMS, fixedState, t1);
  ...
  it("responds to master zoom", () => {
    const a = new Float32Array(GRID_LEN);
    const b = new Float32Array(GRID_LEN);
    computeWarpGrid(1.0, { ...PARAMS, masterZoom: 1.0 }, fixedState, a);
    computeWarpGrid(1.0, { ...PARAMS, masterZoom: 2.0 }, fixedState, b);
    expect(Array.from(a)).not.toEqual(Array.from(b));
  });

  it("produces a varying field across the grid (not a constant)", () => {
    const out = new Float32Array(GRID_LEN);
    computeWarpGrid(1.0, PARAMS, fixedState, out);
    let min = Infinity;
    let max = -Infinity;
    for (const v of out) {
      if (v < min) min = v;
